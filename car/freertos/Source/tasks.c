...
#define vWriteTraceToBuffer()																	\
{
	\if (xTracing)
		...
		\if (uxPreviousTask != pxCurrentTCB->uxTCBNumber)
			\ {
			\if ((pcTraceBuffer + tskSIZE_OF_EACH_TRACE_LINE) <
			      pcTraceBufferEnd)
				\ {
				\uxPreviousTask = pxCurrentTCB->uxTCBNumber;
				\*(unsigned long *)pcTraceBuffer =
				    (unsigned long)xTickCount;
				\pcTraceBuffer += sizeof(unsigned long);
				\*(unsigned long *)pcTraceBuffer =
				    (unsigned long)uxPreviousTask;
				\pcTraceBuffer += sizeof(unsigned long);
				\}
			\
			else
...
#define prvCheckDelayedTasks()																						\
{
	...
	\\while ((pxTCB =
		    (tskTCB *) listGET_OWNER_OF_HEAD_ENTRY(pxDelayedTaskList))
		   != NULL)
		\ {
		\if (xTickCount <
			...
			(&xPendingReadyList,
			 &(pxTCB->xEventListItem)) != pdTRUE)
			    ...
			    /* Is it in the suspended list because it is in the
			       ...
			       specified. */ 
			    if (listIS_CONTAINED_WITHIN
				(NULL, &(pxTCB->xEventListItem)) == pdTRUE)
					       ...
					       const pxEventList) 
{
	    ...
	    pxUnblockedTCB =
	    (tskTCB *) listGET_OWNER_OF_HEAD_ENTRY(pxEventList);
...
static void prvCheckTasksWaitingTermination(void) 
{
...
#if ( INCLUDE_vTaskDelete == 1 )
	{
		    ...
		    if (uxTasksDeleted > (unsigned portBASE_TYPE)0)
			    ...
			    listLIST_IS_EMPTY(&xTasksWaitingTermination);
