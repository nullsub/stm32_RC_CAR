...
static portTickType xCoRoutineTickCount = 0, xLastTickCount = 0, xPassedTicks =
    0;
...
static void prvCheckDelayedList(void) 
{
		    ...
		    while ((pxCRCB =
			    (corCRCB *)
			    listGET_OWNER_OF_HEAD_ENTRY(pxDelayedCoRoutineList))
			   != NULL)
...
static void prvInitialiseCoRoutineLists(void) 
{
		...
		uxPriority++)
				 ...
				 (pxReadyCoRoutineLists[uxPriority]));
...
signed portBASE_TYPE xCoRoutineRemoveFromEventList(const xList * pxEventList) 
{
	    ...
	    pxUnblockedCRCB =
	    (corCRCB *) listGET_OWNER_OF_HEAD_ENTRY(pxEventList);
